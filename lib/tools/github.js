...
async function githubApi(endpoint, options = {}) {
  // In local mode we don't have network access to GitHub; throw early so
  // callers can implement their own local behavior instead of silently
  // failing or hanging.
  if (process.env.RUN_LOCALLY === 'true') {
    throw new Error('GitHub API unavailable in local-first mode');
  }
...
async function getJobStatus(jobId) {
  // Local mode: scan git branches instead of calling GitHub.
  if (process.env.RUN_LOCALLY === 'true') {
    let branches = [];
    try {
      const out = execSync('git branch --list "job/*"', { encoding: 'utf8' });
      branches = out.split('\n').map(l => l.replace(/^[* ]+/, '').trim()).filter(Boolean);
    } catch {}

    const jobs = branches.map(br => {
      const extractedJobId = br.startsWith('job/') ? br.slice(4) : br;
      return {
    ...
    const filtered = jobId ? jobs.filter(j => j.job_id === jobId) : jobs;
    return {
      jobs: filtered,
      queued: 0,
      running: filtered.length,
    };
  }
...
async function fetchJobLog(jobId, commitSha) {
  if (process.env.RUN_LOCALLY === 'true') {
    try {
      const logPath = path.join(process.cwd(), 'logs', jobId, 'session.jsonl');
