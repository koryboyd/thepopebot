...
export async function createModel(options = {}) {
  // Determine provider with optional failover config
  let provider = process.env.LLM_PROVIDER || '';
  if (!provider || provider === 'auto') {
    // try reading config/model-failover.json
    try {
      const cfgPath = path.join(process.cwd(), 'config/model-failover.json');
      const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
      const providers = (cfg.providers || []).slice().sort((a, b) => b.priority - a.priority);
      for (const p of providers) {
        ...
        switch (p.id) {
          case 'anthropic':
            if (process.env.ANTHROPIC_API_KEY) provider = p.id;
            break;
          case 'openai':
          case 'custom':
            if (process.env.OPENAI_API_KEY || process.env.OPENAI_BASE_URL) provider = p.id;
            break;
          case 'google':
            if (process.env.GOOGLE_API_KEY) provider = p.id;
            break;
        }
        if (provider) break;
      }
