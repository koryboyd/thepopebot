...
### What Was Missing
- `lib/skills/` directory existed only as templates; no active registry file was present
- Loader code (`lib/skills/loader.js`) assumed `skills/registry.json` at runtime but there was nothing to load
- Only a handful of skills were registered previously (core-research, email-drafter, task-planner, orchestrator, code-reviewer)
...
- **`lib/skills/registry.json`** — 5-item registry with metadata:
  ...
  [
    {
      "id": "utilities",
      "name": "Utilities",
      "description": "General-purpose utilities including self-modification, browser automation, and credential management.",
      "path": "utilities/SKILL.md"
    },
    {
      "id": "brave-search",
      "name": "brave-search",
      "description": "Perform Brave search queries programmatically.",
  ...
  ```
- **`lib/skills/*/SKILL.md`** — One for each skill with frontmatter (name, description)

### How It Works
1. At server startup, agent prompt builder calls `getCompactSkillList()`
2. Loader reads whichever registry file is available (root `skills/registry.json` for backward compatibility, otherwise `lib/skills/registry.json`)
3. List is injected into agent's system prompt (see Feature 3)
4. Agent can call `loadFullSkill(id)` to read the corresponding SKILL.md file
...
- **Injected context structure:**
  ...
  <available-skills>
  - **Utilities** (utilities): General-purpose utilities including self-modification, browser automation, and credential management
  - **brave-search** (brave-search): Perform Brave searches programmatically
  - **Telegram Integration** (telegram): Interact with users via a Telegram bot
  - **GitHub Actions Helper** (github-actions): Helpers for GitHub Actions workflows
  - **Info Retrieval** (info-retrieval): Fetch and summarize external information
  </available-skills>
  ```
...
Should output 5 skills (or however many you have configured).
